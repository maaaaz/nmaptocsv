...
# Added new features by @mmorenodev (0x98364)
...
output_grp.add_option('-f', '--format', help='CSV column format { fqdn, rdns, hop_number, ip, mac_address, mac_vendor, port, protocol, os, script, service, version, state } (default: ip-fqdn-port-protocol-state-service-version)', default='ip-fqdn-port-protocol-state-service-version', nargs=1)
...
output_grp.add_option('-O', '--open', help='Print only open ports', action='store_true', default=False)
...
#-- Port finding
p_port_without_reason = re.compile('^(?P<number>[\d]+)\/(?P<protocol>tcp|udp)\s+(?P<state>open|open\|filtered|filtered)\s+(?P<service>[\w\S]*)(?:\s*(?P<version>.*))?$')
p_port_with_reason = re.compile('^(?P<number>[\d]+)\/(?P<protocol>tcp|udp)\s+(?P<state>open|open\|filtered|filtered)\s+(?P<service>[\w\S]*)\s+(?P<reason>.* ttl [\d]+)\s*(?:\s*(?P<version>.*))$')
...
def parse_xml(xml_file, select_open):
    """
        Parse the XML file
    ...
    for host in root.findall('host'):
        if 'up' in host.find('status').get('state'):
            ...
            all_ports = ""
            if select_open:
                all_ports = host.findall("./ports/port/state[@state='open']/..")
            else:
                all_ports = host.findall("./ports/port/state/..")

            for port in all_ports:
                protocol = port.get('protocol')
                number = port.get('portid')
                state = port.find('state').get('state')
                #print str(state.get('state')) + ""
...
def main():
    ...
    if not valid_format:
        parser.error("Please specify a valid output format: '%s' is invalid \n\
         Supported objects are { fqdn, rdns, hop_number, ip, mac_address, mac_vendor, port, protocol, os, script, service, version, state }" % ', '.join(unknown_items))
    ...
    if (options.input != None) and (options.xml_input != None):
        ...
        results = parse_xml(options.xml_input, options.open)
